package kodlamaio.hrms.api.controllers;

import java.util.List;

import javax.validation.Valid;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.http.HttpStatus;
import org.springframework.web.bind.MethodArgumentNotValidException;
import org.springframework.web.bind.annotation.CrossOrigin;
import org.springframework.web.bind.annotation.ExceptionHandler;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.ResponseStatus;
import org.springframework.web.bind.annotation.RestController;

import kodlamaio.hrms.business.abstracts.ProgrammingLanguageNameListService;
import kodlamaio.hrms.core.utilities.DataResult;
import kodlamaio.hrms.core.utilities.ErrorDataResult;
import kodlamaio.hrms.core.utilities.Result;
import kodlamaio.hrms.core.validationException.ValidationException;
import kodlamaio.hrms.entities.concretes.ProgrammingLanguageNameList;

@CrossOrigin
@RestController
@RequestMapping("api/programminglanguagenamelist")
public class ProgramminglanguageNameListControllers {
	
	private ProgrammingLanguageNameListService programmingLanguageNameListService;
	private ValidationException validationException;
	
	@Autowired
	public ProgramminglanguageNameListControllers(ProgrammingLanguageNameListService programmingLanguageNameListService,ValidationException validationException) {
		super();
		this.programmingLanguageNameListService = programmingLanguageNameListService;
		this.validationException=validationException;
	}
	
	@PostMapping("/add")
	public Result add(@Valid @RequestBody ProgrammingLanguageNameList programmingLanguageNameList) {
		return this.programmingLanguageNameListService.add(programmingLanguageNameList);
	}
	
	@GetMapping("/getAll")
	public DataResult<List<ProgrammingLanguageNameList>> getAll() {
		return this.programmingLanguageNameListService.getAll();
	}
	
	@GetMapping("/findById")
	public DataResult<List<ProgrammingLanguageNameList>> findById(int id) {
		return this.programmingLanguageNameListService.findById(id);
	}
	
	
	@ExceptionHandler(MethodArgumentNotValidException.class)
	@ResponseStatus(HttpStatus.BAD_REQUEST)
	public ErrorDataResult<Object> validation(MethodArgumentNotValidException exceptions) {
		return validationException.handleValidationException(exceptions);
	}
	
}
